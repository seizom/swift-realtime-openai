...
public final class Conversation: @unchecked Sendable {
    ...
    public var api: RealtimeAPI {
        RealtimeAPI(connector: client)
    }
    ...
    /// Send a raw client event payload to the server.
    /// Use this to emit custom events that are not currently modeled by `ClientEvent`.
    /// - Parameter payload: A JSON-serializable dictionary representing the event.
    public func send(rawEvent payload: [String: Any]) throws {
        guard JSONSerialization.isValidJSONObject(payload) else {
            throw ConversationError.invalidEventPayload
        }

        do {
            try client.sendRawEvent(payload)
        } catch {
            throw error
        }
    }
